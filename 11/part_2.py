...
def get_input():
    return open('11/input.txt').read().splitlines()
...
class Monkey():
    ...
    def do_round(self):
        self.current_item = self.items[0] % 9699690 # check helper, too lazy to implement
        del self.items[0]
...
def generate_monkeys(monkeys):
    ...
    while i < len(instructions):
        ...
        b = [int(x) for x in ITEMS.findall(instructions[i + 1])[0].split(',')]
...
def ass_1():
    ...
    for x in range(10000):
        ...
        if x+1 in (20,1000,2000,3000,4000,5000,6000,7000,8000,9000,10000):
            print([monkeys[key].no_items_passed for key in monkeys.keys()])
